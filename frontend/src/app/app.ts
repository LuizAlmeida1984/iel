...
export class App {
    ...
    /* protected async handleSave(): Promise<void> {
        ...
        try {
            ...
            console.log('139 result', result);
            console.log('139 filtered', filtered);
    ...
    private safeParseAI(result: any): any {
        try {
            // 1) Se já vier objeto, não precisa parsear
            const raw = typeof result === 'string' ? JSON.parse(result) : result;

            // 2) pega o campo analysis
            let text = raw?.analysis ?? raw;
            ...
            // 4) tenta parsear JSON interno
            return JSON.parse(text);

        } catch (err) {
            console.error('Erro no safeParseAI:', err);
    ...
    protected async exportPdf(): Promise<void> {
        ...
        try {
            ...
            if (radarImage) {
                documentPdf.setFont('helvetica', 'bold');
                documentPdf.text('Gráfico Radar', 14, y);
                y += 6;

                documentPdf.addImage(radarImage, 'PNG', 14, y, 85, 85);
                y += 90;
            }
            ...
            documentPdf.setFont('helvetica', 'bold');
            documentPdf.setFontSize(12);
            documentPdf.text('Parecer de Mentoria', 14, y);
            y += 8;
            ...
            documentPdf.save(`parecer-mentoria-${safeName}-${fileDate}.pdf`);
    ...
    normalize(value: any): any[] {
        ...
        // array → retorna direto
        if (Array.isArray(value)) return value;

        // objeto → transforma em array de entries
        if (typeof value === 'object') return Object.entries(value);

        // string / number / boolean → vira array com 1 item
        return [value];
    }
